...
app.view.open = function(events, data, form) {
  ...
  _open.fillTable = function(table) {
    _initialized || _open.uninitialized();
    ...
    Array.prototype.forEach.call(Object.keys(data), function(id) {
      var row = control.renderRow(trow_tpl, data[id]);
      _rows += row.outerHTML;
    });
...
app.view.unload = function() {
  ...
  if (control.temp.form_submit || control.temp.form_changes === JSON.stringify(app.view.getFormData(control.temp.form_elements))) {
    return;
  }
